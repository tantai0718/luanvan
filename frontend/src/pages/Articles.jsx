      ...
      <section className="space-y-10 lg:col-span-8">
        {loading ? <div className="flex justify-center py-24"><div className="h-10 w-10 animate-spin rounded-full border-4 border-primary border-t-transparent" /></div> : featured ? <>
          <Link to={`/articles/${featured.ma_bai_viet}`} className="group relative block overflow-hidden rounded-xl bg-card shadow-lg">
            <div className="aspect-[16/9] overflow-hidden">{featured.hinh_anh ? <img src={resolveImg(featured.hinh_anh)} alt={featured.tieu_de} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center bg-emerald-50"><FileText size={72} className="text-primary/30" /></div>}</div><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
