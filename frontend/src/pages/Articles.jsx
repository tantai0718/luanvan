...
const categoryMap = {
  quy_trinh: { label: 'Quy trình', icon: 'eco' },
  suc_khoe: { label: 'Sức khỏe', icon: 'favorite' },
  am_thuc: { label: 'Ẩm thực', icon: 'restaurant' },
  kinh_nghiem: { label: 'Kinh nghiệm', icon: 'lightbulb' },
  khac: { label: 'Khác', icon: 'article' },
};
const category = (key) => categoryMap[key] || categoryMap.khac;
    ...
    <div className="mx-auto max-w-[1280px] px-6 pb-16"><div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
      <section className="space-y-10 lg:col-span-8">
        {loading ? <div className="flex justify-center py-24"><div className="h-10 w-10 animate-spin rounded-full border-4 border-primary border-t-transparent" /></div> : featured ? <>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">{items.filter((item) => item.ma_bai_viet !== featured.ma_bai_viet).map((item) => <Link key={item.ma_bai_viet} to={`/articles/${item.ma_bai_viet}`} className="group overflow-hidden rounded-xl border border-outline-variant bg-white shadow-sm transition hover:shadow-md"><div className="aspect-video overflow-hidden">{item.hinh_anh ? <img src={item.hinh_anh} alt={item.tieu_de} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center bg-emerald-50"><span className="material-symbols-outlined text-5xl text-primary/25">{category(item.the_loai).icon}</span></div>}</div><div className="p-5"><p className="mb-2 text-xs font-bold text-primary">{category(item.the_loai).label.toUpperCase()} <span className="mx-1 text-outline">•</span><span className="font-normal text-on-surface-variant">{date(item.ngay_dang)}</span></p><h2 className="font-h3 text-lg font-bold leading-snug text-on-surface group-hover:text-primary">{item.tieu_de}</h2>{item.tom_tat && <p className="mt-3 line-clamp-3 text-sm text-on-surface-variant">{item.tom_tat}</p>}</div></Link>)}</div>
