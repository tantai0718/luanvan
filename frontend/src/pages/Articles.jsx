...
const categoryIcon = { 'Tin Tức Nông Sản': 'eco', 'Mẹo Bảo Quản': 'inventory_2' };
const icon = (name) => categoryIcon[name] || 'article';
      ...
      {categories.length > 0 && (
        <div className="mb-8 flex flex-wrap gap-3">
          <button onClick={() => setActiveCate('')}
            className={`px-5 py-2.5 rounded-full text-body-sm font-bold transition-all ${!activeCate ? 'bg-primary text-on-primary shadow-md' : 'bg-white border border-outline-variant text-on-surface-variant hover:border-primary hover:text-primary'}`}>
            Tất cả
          </button>
          {categories.map(c => (
            <button key={c.madm} onClick={() => setActiveCate(c.madm)}
              className={`px-5 py-2.5 rounded-full text-body-sm font-bold transition-all flex items-center gap-2 ${String(activeCate) === String(c.madm) ? 'bg-primary text-on-primary shadow-md' : 'bg-white border border-outline-variant text-on-surface-variant hover:border-primary hover:text-primary'}`}>
              <span className="material-symbols-outlined text-base">{icon(c.ten_danh_muc)}</span>
              {c.ten_danh_muc}
            </button>
          ))}
        </div>
      ...
      <section className="space-y-10 lg:col-span-8">
        {loading ? <div className="flex justify-center py-24"><div className="h-10 w-10 animate-spin rounded-full border-4 border-primary border-t-transparent" /></div> : featured ? <>
          <Link to={`/articles/${featured.ma_bai_viet}`} className="group relative block overflow-hidden rounded-xl bg-surface-container-lowest shadow-lg">
            <div className="aspect-[16/9] overflow-hidden">{featured.hinh_anh ? <img src={featured.hinh_anh} alt={featured.tieu_de} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center bg-emerald-50"><span className="material-symbols-outlined text-7xl text-primary/30">{icon(featured.ten_danh_muc)}</span></div>}</div><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">{items.filter((item) => item.ma_bai_viet !== featured.ma_bai_viet).map((item) => <Link key={item.ma_bai_viet} to={`/articles/${item.ma_bai_viet}`} className="group overflow-hidden rounded-xl border border-outline-variant bg-white shadow-sm transition hover:shadow-md"><div className="aspect-video overflow-hidden">{item.hinh_anh ? <img src={item.hinh_anh} alt={item.tieu_de} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center bg-emerald-50"><span className="material-symbols-outlined text-5xl text-primary/25">{icon(item.ten_danh_muc)}</span></div>}</div><div className="p-5"><p className="mb-2 text-xs font-bold text-primary">{(item.ten_danh_muc || 'Khác').toUpperCase()} <span className="mx-1 text-outline">•</span><span className="font-normal text-on-surface-variant">{date(item.ngay_tao)}</span></p><h2 className="font-h3 text-lg font-bold leading-snug text-on-surface group-hover:text-primary">{item.tieu_de}</h2>{item.tom_tat && <p className="mt-3 line-clamp-3 text-sm text-on-surface-variant">{item.tom_tat}</p>}</div></Link>)}</div>
