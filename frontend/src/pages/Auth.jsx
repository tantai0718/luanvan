...
import { AlertCircle, Mail, Lock, User, Phone, ShieldCheck } from 'lucide-react';
...
function FloatingInput({ label, icon, ...props }) {
  ...
  return (
    <div className="relative pt-3">
      <span className="absolute left-1 bottom-3 text-text-secondary flex items-center justify-center">
        {icon}
      </span>
      ...
      <input
        ...
        className={`w-full border-b bg-transparent pb-2.5 pl-10 pr-3 text-base font-medium text-text-primary outline-none transition-all duration-300 placeholder:text-transparent ${
          focused ? 'border-primary' : 'border-border hover:border-text-secondary'
        }`}
        placeholder=" "
      />
      ...
      <label
        className={`absolute transition-all duration-200 pointer-events-none ${
          isActive
            ? '-top-1 left-10 text-[11px] font-bold tracking-wider uppercase text-primary'
            : 'top-6 left-10 text-base text-text-secondary'
        }`}
      >
        {label}
      </label>
...
function AuthLayout({ children }) {
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-5xl bg-white rounded-3xl shadow-xl overflow-hidden flex min-h-[600px] border border-border">
  ...
  return (
    <AuthLayout>
      <div className="md:hidden mb-6">
        <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3.5 py-1.5">
          <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
          <span className="text-xs font-bold tracking-widest uppercase text-primary">Chợ Nông Sản</span>
        </div>
      </div>

      <p className="text-xs font-bold tracking-widest uppercase text-primary mb-1.5">Chào mừng trở lại</p>
      <h2 className="text-3xl font-bold text-text-primary tracking-tight">Đăng nhập</h2>
      <p className="mt-1.5 text-text-secondary text-base">Nhập thông tin để tiếp tục mua sắm nông sản sạch.</p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        ...
        <button
          disabled={loading}
          className="w-full mt-4 rounded-xl bg-primary py-3.5 text-base font-bold text-white transition-all duration-200 hover:bg-primary/90 hover:shadow-lg hover:shadow-primary/20 active:scale-[0.99] disabled:opacity-60 flex items-center justify-center gap-2"
        >
          {loading ? (
      ...
      <div className="mt-8 relative">
        ...
        <div className="relative flex justify-center">
          <span className="bg-white px-3 text-xs font-bold tracking-widest uppercase text-text-secondary">hoặc</span>
        </div>
      </div>
...
export function Register() {
  ...
  return (
    <AuthLayout>
      ...
      <p className="text-xs font-bold tracking-widest uppercase text-primary mb-1.5">Tạo mới tài khoản</p>
      <h2 className="text-3xl font-bold text-text-primary tracking-tight">Đăng ký</h2>
      <p className="mt-1.5 text-text-secondary text-base">Bắt đầu hành trình mua sắm nông sản sạch.</p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-5">
        ...
        <button
          disabled={loading}
          className="w-full mt-4 rounded-xl bg-secondary py-3.5 text-base font-bold text-white transition-all duration-200 hover:bg-secondary/90 hover:shadow-lg hover:shadow-secondary/20 active:scale-[0.99] disabled:opacity-60 flex items-center justify-center gap-2"
        >
          {loading ? (
            <div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent" />
          ) : (
