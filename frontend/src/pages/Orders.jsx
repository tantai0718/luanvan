...
const canCancelOrder = (status) =>
  ["cho_xac_nhan", "da_xac_nhan"].includes(status);
...
export function OrderList() {
  ...
  const handleCancelSubscription = async (id) => {
    if (!window.confirm("Bạn có chắc muốn hủy đăng ký giao định kỳ này không?"))
      return;
    ...
    try {
      ...
      setSubscriptions((prev) =>
        prev.map((item) =>
          item.ma_dang_ky === id ? { ...item, trang_thai: "da_huy" } : item,
        ),
      );
    } finally {
    ...
    <div className="market-shell pb-10">
      <div className="market-page space-y-6 py-6">
        <section className="market-panel p-6">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#2d9e63]">
            Đơn hàng của tôi
          </p>
          <h1 className="mt-2 text-4xl font-bold text-slate-900">
            Theo dõi toàn bộ đơn hàng và lịch giao trong một nơi
          </h1>
          <p className="mt-3 text-sm leading-7 text-slate-600">
            Gồm đơn thường, đơn đặt trước và các đăng ký giao định kỳ mà bạn
        ...
        {loading ? (
          <div className="space-y-4">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="h-28 animate-pulse rounded-[18px] bg-[#f3f7f4]"
              />
            ))}
          </div>
        ) : (
          <>
                  ...
                  {subscriptions.map((subscription) => {
                    ...
                    return (
                      <div
                        ...
                        className="rounded-[18px] border border-[#dce7df] bg-white p-4"
                      >
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span
                                className={`rounded-full px-2.5 py-1 text-xs font-semibold ${status.color}`}
                              >
                                {status.label}
                              </span>
                            </div>
            ...
            <section className="space-y-4">
              {orders.length ? (
                orders.map((order) => {
                  const status =
                    orderStatusMap[order.trang_thai] || orderStatusMap.da_huy;
                  const orderType =
                    ...
                    >
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span
                              className={`rounded-full px-2.5 py-1 text-xs font-semibold ${orderType.color}`}
                            >
                              {orderType.label}
                            </span>
                            <span
                          ...
                          {order.loai_don === "dat_truoc" ? (
                            ...
                            <div className="mt-2 rounded-lg bg-[#eef8f3] border border-[#b8e0c6] px-3 py-2">
                        ...
                        <div className="text-left lg:text-right">
                          ...
                          <p className="mt-1 text-2xl font-bold text-[#1a7a4a]">
                      ...
                      <div className="mt-4 flex flex-col gap-3 border-t border-[#edf2ee] pt-4 sm:flex-row sm:items-center sm:justify-between">
                        <span className="text-sm text-slate-500">
                          Nhấn để xem chi tiết và tiến độ giao hàng.
                        </span>
                        {canCancelOrder(order.trang_thai) ? (
                          <button
                            onClick={(event) =>
                              handleCancelOrder(event, order.ma_don_hang)
                            }
                            disabled={cancelingOrderId === order.ma_don_hang}
                            className="rounded-full bg-red-50 px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-100 disabled:opacity-60"
                          >
                            {cancelingOrderId === order.ma_don_hang
                              ? "Đang hủy..."
                              : "Hủy đơn"}
                          </button>
                        ) : null}
...
export function OrderDetail() {
  ...
  const orderType =
    orderTypeMap[order.loai_don || "thuong"] || orderTypeMap.thuong;
        ...
        <section className="market-panel p-6">
                ...
                <div className="mt-4 rounded-xl bg-orange-50 border border-orange-200 p-4">
                  ...
                  <p className="text-sm text-orange-700">
                    ...
                    <b>
                      {" "}
                      {order.ngay_giao_du_kien
                        ? new Date(order.ngay_giao_du_kien).toLocaleDateString(
                          "vi-VN",
                        )
                        : "Chưa xác định"}
                    </b>
                  ...
                  <p className="mt-2 text-xs text-orange-600">
                    Chúng tôi sẽ liên hệ khi sản phẩm sẵn sàng để giao.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="mt-6 space-y-3">
            {items.map((item) => (
              <div
                key={item.mactdh}
                className="flex items-center gap-4 rounded-[18px] border border-[#dce7df] bg-white p-4"
              >
                <img
                  src={
                    item.hinh_san_pham
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-[18px] bg-[#f3f7f4] p-4">
              ...
              <p className="mt-2 text-sm leading-7 text-slate-500">
                {order.ghi_chu || "Không có ghi chú thêm."}
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-[18px] border border-[#dce7df] p-4">
            ...
            {order.ghi_chu_khuyen_mai ? (
              <p className="mt-2 rounded-lg bg-[#eef8f3] p-3 text-xs leading-5 text-[#1a7a4a]">
                {order.ghi_chu_khuyen_mai}
              </p>
            ) : null}
            <div className="mt-2 flex justify-between text-sm text-slate-500">
              <span>Phí vận chuyển</span>
              <span>{formatCurrency(Number(order.tong_thanh_toan || 0) - subtotal + Number(order.giam_gia || 0))}</span>
            </div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <Link
              to="/orders"
              className="flex-1 rounded-full border border-[#dce7df] py-3 text-center text-sm font-semibold text-slate-700 hover:bg-[#f3f7f4]"
            >
              ← Về danh sách đơn
            </Link>
            {canCancelOrder(order.trang_thai) ? (
              ...
              <Link
                to="/products"
                className="flex-1 rounded-full bg-[#1a7a4a] py-3 text-center text-sm font-semibold text-white hover:bg-[#14633b]"
              >
                Mua thêm sản phẩm
              </Link>
            )}
