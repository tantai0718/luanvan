    ...
    <div className="market-shell pb-10">
      <div className="market-page space-y-6 py-6">
        <section className="market-panel p-6">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Hồ sơ người dùng</p>
          <h1 className="mt-2 text-4xl font-bold text-text-primary">Cập nhật thông tin tài khoản và bảo mật</h1>
        ...
        {(message || error) ? (
          <div className={`market-panel px-5 py-4 text-sm ${error ? 'text-danger' : 'text-primary'}`}>
            {error || message}
          </div>
        ) : null}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <form onSubmit={handleProfileSave} className="market-panel p-6">
            <h2 className="text-2xl font-bold text-text-primary">Thông tin cá nhân</h2>
            <div className="mt-5 space-y-4">
              ...
              <input
                value={user?.email || ''}
                disabled
                className="w-full rounded-2xl border border-border bg-background px-4 py-3 text-sm text-text-secondary/60"
              />
              <input
          ...
          <form onSubmit={handlePasswordSave} className="market-panel p-6">
            <h2 className="text-2xl font-bold text-text-primary">Đổi mật khẩu</h2>
            <div className="mt-5 space-y-4">
              <input
                type="password"
              ...
              <button className="rounded-full bg-secondary px-5 py-3 text-sm font-semibold text-white hover:bg-secondary/90">
